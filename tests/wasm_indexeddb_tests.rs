...
use ifecaro::services::indexeddb::{
    clear_all_disabled_choices_from_indexeddb, get_choice_from_indexeddb,
    get_disabled_choices_from_indexeddb, set_choices_to_indexeddb,
...
async fn test_indexeddb_choices_full_path_real() {
    ...
    set_choices_to_indexeddb(chapter_id, &empty_arr)
        .await
        .unwrap();
...
async fn test_disabled_choices_storage() {
    ...
    let result_js = get_disabled_choices_from_indexeddb(paragraph_id)
        .await
        .unwrap();
    let result_arr = Array::from(&result_js);
    let result: Vec<u32> = result_arr
        .iter()
    ...
    console::log_1(&format!("Final result: {:?}", result).into());
    assert!(
        result.contains(&choice_index),
        "Disabled choice should be stored in IndexedDB"
    );
...
async fn test_disabled_choices_persistence() {
    ...
    let result_js = get_disabled_choices_from_indexeddb(paragraph_id)
        ...
        .unwrap();
    let result_arr = Array::from(&result_js);
    let mut result: Vec<u32> = result_arr
        .iter()
        .filter_map(|v| v.as_f64().map(|n| n as u32))
        .collect();
...
async fn test_clear_all_disabled_choices_functionality() {
    ...
    // Verify it exists
    let pre_clear_js = get_disabled_choices_from_indexeddb(paragraph_id)
        .await
        .unwrap();
    let pre_clear_arr = Array::from(&pre_clear_js);
    assert!(
        pre_clear_arr.length() > 0,
        "Disabled choice should exist before clearing"
    );
    ...
    let post_clear_js = get_disabled_choices_from_indexeddb(paragraph_id)
        .await
        .unwrap();
    let post_clear_arr = Array::from(&post_clear_js);
    assert_eq!(
        post_clear_arr.length(),
        0,
        "Disabled choices should be cleared from IndexedDB"
    );
}
