...
use ifecaro::contexts::chapter_context::{Chapter, ChapterState, ChapterTitle};
use ifecaro::contexts::paragraph_context::{Paragraph, ParagraphChoice, ParagraphState, Text};
...
mod interaction_tests {
    ...
    fn test_chapter_selection_logic() {
        ...
        let chapter = chapter_state
            .chapters
            .iter()
            .find(|c| c.id == selected_chapter)
            .unwrap();
        let title = chapter
            .titles
            .iter()
    ...
    fn test_paragraph_editing_logic() {
        ...
        let existing_text = selected_paragraph
            .texts
            .iter()
            .find(|t| t.lang == current_language)
            .unwrap();
    ...
    fn test_form_validation_logic() {
        ...
        for (content, chapter, choices, expected) in test_cases {
            ...
            let choices_valid = choices.iter().all(
                |(
                    _choice_text,
                    to,
                    _type,
    ...
    fn test_edit_mode_language_switching_content_update() {
        ...
        let initial_text = selected_paragraph
            .texts
            .iter()
        ...
        let same_text = selected_paragraph
            .texts
            .iter()
            .find(|t| t.lang == current_content_language)
            .unwrap();
        ...
        let english_text = selected_paragraph
            .texts
            .iter()
            .find(|t| t.lang == current_content_language)
            .unwrap();
    ...
    fn test_form_submit_button_state_management() {
        ...
        let check_submit_enabled = |content: &str,
                                    chapter: &str,
                                    choices: &Vec<(
            String,
            Vec<String>,
    ...
    fn test_edit_mode_comprehensive_language_switch_content_update() {
        ...
        impl ComprehensiveEditState {
            ...
            fn update_ui_language(&mut self, new_lang: String) {
                ...
                self.ui_labels.insert(
                    "content_label".to_string(),
                    self.get_ui_text("content_label", &new_lang),
                );
                self.ui_labels.insert(
                    "chapter_label".to_string(),
                    self.get_ui_text("chapter_label", &new_lang),
                );
                self.ui_labels.insert(
                    "choices_label".to_string(),
                    self.get_ui_text("choices_label", &new_lang),
                );

                self.button_texts.insert(
                    "save_button".to_string(),
                    self.get_ui_text("save_button", &new_lang),
                );
                self.button_texts.insert(
                    "cancel_button".to_string(),
                    self.get_ui_text("cancel_button", &new_lang),
                );

                self.placeholder_texts.insert(
                    "content_placeholder".to_string(),
                    self.get_ui_text("content_placeholder", &new_lang),
                );
                self.placeholder_texts.insert(
                    "choice_placeholder".to_string(),
                    self.get_ui_text("choice_placeholder", &new_lang),
                );

                self.error_messages.insert(
                    "required_error".to_string(),
                    self.get_ui_text("required_error", &new_lang),
                );
                self.error_messages.insert(
                    "invalid_choice_error".to_string(),
                    self.get_ui_text("invalid_choice_error", &new_lang),
                );
            }

            fn update_content_language(
                &mut self,
                new_lang: String,
        ...
        assert_eq!(
            edit_state.ui_labels.get("content_label").unwrap(),
            "段落內容"
        );
        assert_eq!(
            edit_state.ui_labels.get("chapter_label").unwrap(),
            "選擇章節"
        );
        ...
        assert_eq!(
            edit_state
                .placeholder_texts
                .get("content_placeholder")
                .unwrap(),
            "請輸入段落內容..."
        );
        assert_eq!(
            edit_state.error_messages.get("required_error").unwrap(),
            "此欄位為必填"
        );
        ...
        let chapter_title = edit_state.get_chapter_title(
            &edit_state.selected_chapter_id,
            &edit_state.current_ui_language,
            &chapter_state,
        );
        ...
        let chapter_title_en = edit_state.get_chapter_title(
            &edit_state.selected_chapter_id,
            &edit_state.current_ui_language,
            &chapter_state,
        );
        ...
        let chapter_title_zh_again = edit_state.get_chapter_title(
            &edit_state.selected_chapter_id,
            &edit_state.current_ui_language,
            &chapter_state,
        );
    ...
    fn test_invalid_target_references() {
        let paragraph_state = ParagraphState {
            paragraphs: vec![Paragraph {
                ...
                choices: vec![ParagraphChoice::Simple(
                    vec!["nonexistent_para".to_string()],
                )],
            }],
            loaded: true,
        };
        ...
        let paragraph_state = ParagraphState {
            paragraphs: vec![
                Paragraph {
                    ...
                    choices: vec![ParagraphChoice::Simple(vec!["para_b".to_string()])],
                },
                Paragraph {
                    id: "para_b".to_string(),
                    chapter_id: "chapter1".to_string(),
                    ...
                    choices: vec![ParagraphChoice::Simple(vec!["para_a".to_string()])],
                },
            ],
            loaded: true,
        };
