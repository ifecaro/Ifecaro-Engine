...
mod story_integration_tests {
    use super::*;
    use ifecaro::pages::story::{merge_paragraphs_for_lang, Paragraph, Text};
    ...
    fn test_story_ui_integration() {
        use ifecaro::components::story_content::{
            Action, Choice, StoryContentUI, StoryContentUIProps,
        };
...
mod ui_integration_tests {
    ...
    fn test_keyboard_state_integration() {
        ...
        keyboard_state.enabled_choices =
            std::rc::Rc::new(["p1".to_string(), "p2".to_string()].into_iter().collect());
