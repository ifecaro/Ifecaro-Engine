...
mod reader_mode_integration_tests {
    use ifecaro::pages::story::{merge_paragraphs_for_lang, ComplexChoice, Paragraph, Text};
    ...
    fn create_paragraph_with_multiple_choices(
        ...
        let complex_choices: Vec<ComplexChoice> = choice_targets
            .into_iter()
            .map(|targets| ComplexChoice {
    ...
    fn create_paragraph_with_no_choices(
        id: &str,
        chapter_id: &str,
        lang: &str,
        text: &str,
    ...
    fn test_reader_mode_story_network_expansion() {
        ...
        assert!(
            expanded_path.len() >= 2,
            "Should expand at least 2 paragraphs, got {}",
            expanded_path.len()
        );
    ...
    fn test_reader_mode_handles_empty_choice_targets() {
        ...
        let result = merge_paragraphs_for_lang(&expanded, "en", true, false, &[]);
    ...
    fn test_reader_mode_loop_prevention() {
        ...
        assert!(
            expanded.len() <= 2,
            "Should prevent infinite loops, got {} paragraphs",
            expanded.len()
        );
        ...
        for (id, count) in id_counts {
            assert_eq!(
                count, 1,
                "Paragraph {} should appear only once, appeared {} times",
                id, count
            );
        }
    }
    ...
    fn test_reader_mode_language_consistency() {
        ...
        let en_result = merge_paragraphs_for_lang(&paragraphs, "en", true, false, &[]);
        ...
        let zh_result = merge_paragraphs_for_lang(&paragraphs, "zh", true, false, &[]);
    ...
    fn test_reader_mode_performance_with_large_path() {
        ...
        let result = merge_paragraphs_for_lang(&paragraphs, "en", true, false, &[]);
        ...
        assert_eq!(
            separator_count, 49,
            "Should have 49 separators between 50 paragraphs"
        );
    }
    ...
    fn test_choices_append_not_overwrite() {
        ...
        let p2 = "p2";
        // 第一次選擇
        if !choices.contains(&p1.to_string()) {
            choices.push(p1.to_string());
        }
        // 第二次選擇
        if !choices.contains(&p2.to_string()) {
            choices.push(p2.to_string());
        }
        // 再次選擇 p1 不應重複
        if !choices.contains(&p1.to_string()) {
            choices.push(p1.to_string());
    ...
    fn test_reload_auto_expand_from_choices() {
        ...
        let expanded: Vec<Paragraph> = choice_ids
            .iter()
    ...
    fn test_multi_target_only_in_random_choices() {
        ...
        // choices 不變
        // random_choices 寫入
        random_choices.insert(
            format!("{}:{}", paragraph_id, choice_index),
            selected.to_string(),
        );
        assert!(choices.is_empty());
        assert_eq!(
            random_choices.get(&format!("{}:{}", paragraph_id, choice_index)),
            Some(&"treasure".to_string())
        );
    }
