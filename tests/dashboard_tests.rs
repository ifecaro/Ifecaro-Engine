...
use ifecaro::pages::dashboard::{
    ChapterData, Collection, Dashboard, DashboardProps, Data, SystemData,
};
        ...
        ParagraphState {
            paragraphs: vec![
                ...
                Paragraph {
                    ...
                    choices: vec![ParagraphChoice::Simple(vec!["para3".to_string()])],
                },
                Paragraph {
                    id: "para3".to_string(),
...
mod unit_tests {
    ...
    fn test_language_state_data() {
        let (current_lang, default_lang) =
            dashboard_test_helpers::create_test_language_state_data();
    ...
    fn test_paragraph_text_languages() {
        ...
        assert_eq!(
            en_text.unwrap().paragraphs,
            "This is the first test paragraph content."
        );
    }
...
mod integration_tests {
    ...
    fn test_paragraph_content_translation() {
        ...
        let zh_content = paragraph
            .texts
            .iter()
            .find(|t| t.lang == "zh-TW")
            .map(|t| &t.paragraphs);

        let en_content = paragraph
            .texts
            .iter()
            .find(|t| t.lang == "en-US")
            .map(|t| &t.paragraphs);
    ...
    fn test_choice_localization() {
        ...
        let zh_choices = paragraph
            .texts
            .iter()
            .find(|t| t.lang == "zh-TW")
            .map(|t| &t.choices);

        let en_choices = paragraph
            .texts
            .iter()
            .find(|t| t.lang == "en-US")
            .map(|t| &t.choices);
    ...
    fn test_chapter_title_localization() {
        ...
        let zh_title = chapter
            .titles
            .iter()
            .find(|t| t.lang == "zh-TW")
            .map(|t| &t.title);

        let en_title = chapter
            .titles
            .iter()
            .find(|t| t.lang == "en-US")
            .map(|t| &t.title);
...
mod error_handling_tests {
    ...
    fn test_missing_translation_handling() {
        ...
        paragraph_state.paragraphs[0]
            .texts
            .retain(|t| t.lang != "en-US");
    ...
    fn test_large_dataset_performance() {
        ...
        for i in 0..1000 {
            paragraph_state.paragraphs.push(Paragraph {
                ...
                choices: vec![ParagraphChoice::Simple(vec![format!("para{}", i + 1)])],
            });
        }
        ...
        assert!(
            ...
            "Chapter paragraph retrieval took too long: {:?}",
            duration
        );
...
mod accessibility_tests {
    ...
    fn test_content_structure_accessibility() {
        ...
        for paragraph in &paragraph_state.paragraphs {
            ...
            assert!(
                !paragraph.chapter_id.is_empty(),
                "Chapter ID should not be empty"
            );
            assert!(
                !paragraph.texts.is_empty(),
                "Paragraph should have at least one text version"
            );
...
mod api_tests {
    ...
    fn test_data_structure_compatibility() {
        ...
        let data = Data {
            items: vec![
                dashboard_test_helpers::create_test_paragraph_state().paragraphs[0].clone(),
            ],
        };
...
mod ui_state_tests {
    ...
    fn test_form_state_validation() {
        ...
        let main_fields_valid =
            !paragraphs_content.trim().is_empty() && !selected_chapter.is_empty();
        ...
        let choices_valid = choices.iter().all(
            |(_choice_text, to, type_, _key, _value, _target_chapter, _same_page, _time_limit)| {
    ...
    fn test_real_time_form_validation() {
        ...
        let validate_form = |content: &str,
                             chapter: &str,
                             choices: &Vec<(String, Vec<String>, String)>|
         -> (bool, Vec<String>) {
    ...
    fn test_submit_button_dynamic_state() {
        ...
        impl FormState {
            fn can_submit(&self) -> bool {
                let basic_valid =
                    !self.content.trim().is_empty() && !self.chapter.trim().is_empty();
                let choices_valid = self
                    .choices
                    .iter()
                    .all(|(text, targets)| !text.trim().is_empty() && !targets.is_empty());
