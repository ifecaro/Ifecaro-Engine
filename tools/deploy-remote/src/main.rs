...
fn rewrite_staging_base_url_on_remote(
    ...
    println!(
        "✅ Rewrote staging API base URL in remote frontend volume for compose project ({})",
        compose_project_name
    );
    Ok(())
}
...
fn build_staging_base_url_rewrite_command(compose_project_name: &str) -> String {
    let inner_script = r#"index=/shared/index.html && [ -f "$index" ] && sed -i -e "s|https://ifecaro.com/db/api|https://ifecaro.com/staging/db/api|g" -e "s|\"/db/api\"|\"/staging/db/api\"|g" -e "s|'/db/api'|'/staging/db/api'|g" -e "s|\"/assets/|\"/staging/assets/|g" -e "s|'/assets/|'/staging/assets/|g" -e "s|=/assets/|=/staging/assets/|g" -e "s|\"/tailwind.css\"|\"/staging/tailwind.css\"|g" -e "s|'/tailwind.css'|'/staging/tailwind.css'|g" -e "s|=/tailwind.css|=/staging/tailwind.css|g" -e "s|\"/manifest.json\"|\"/staging/manifest.json\"|g" -e "s|'/manifest.json'|'/staging/manifest.json'|g" -e "s|=/manifest.json|=/staging/manifest.json|g" "$index""#;
    let volume_name = resolve_staging_frontend_assets_volume_name(compose_project_name);

    format!(
        "docker run --rm -v {}:/shared alpine:3.20 sh -lc {}",
        shell_escape(&volume_name),
...
mod tests {
  ...
  "build_time": "2026-01-01T00:00:00Z"
    ...
    fn staging_base_url_rewrite_command_targets_shared_index_and_volume() {
        ...
        assert!(command.contains(r#"\"/tailwind.css\"|\"/staging/tailwind.css\""#));
        assert!(command.contains("/staging/tailwind.css"));
        assert!(command.contains("s|=/tailwind.css|=/staging/tailwind.css|g"));
